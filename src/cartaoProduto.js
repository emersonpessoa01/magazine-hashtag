...
export function renderizarCatalogo() {
  for (const produtoCatalago of catalogo) {
    const { id, imagem, nome, marca, preco } = produtoCatalago;
    const cartaoProduto = `<div class="border-solid w-48 m-2 flex flex-col p-2 justify-between group shadow-xl shadow-slate-400 rounded-lg" id="card-produto-${id}">
    <div class="h-full h-full overflow-hidden rounded-lg">
      ...
      <p class="text-sm">${marca}</p>
      <p class="text-sm">${preco}</p>
      <button id="adicionar-${id}" class="bg-slate-950 hover:bg-slate-700 duration-500 text-slate-200"><i class="fa-solid fa-cart-plus"></i></button>
    </divc> `;
